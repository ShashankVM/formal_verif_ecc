module rvecc_sva  #(
    ...
    ASSERT_NO_ERROR_PRESENT_RECEIVED_DATA:        assert property ((!single_error_inject && !double_error_inject) |-> (encoded_data == received_data));

    // prove that if no error injected then encoded data matches the decoded data
    ASSERT_NO_ERROR_PRESENT_DECODED_DATA:        assert property ((!single_error_inject && !double_error_inject) |-> (encoded_data == decoded_data));
    ...
    ASSERT_DOUBLE_ERROR_DETECTION_DOUBLE_ECC_ERROR:   assert property (double_error_inject && decoder_en |-> double_ecc_error);

    // prove that when double-errors are injected, single_ecc_error remains low
    ASSERT_DOUBLE_ERROR_DETECTION_NO_SINGLE_ECC_ERROR:   assert property (double_error_inject && decoder_en |-> !single_ecc_error);
    ...
    // adding constraint on error position for single error injection due to issue: https://github.com/chipsalliance/Cores-VeeR-EL2/issues/521#issue-5043157270
    ASSERT_SINGLE_ERROR_CORRECTION_IN_SED_DED_DOUBLE_ECC_ERROR:   assert property (sed_ded && single_error_inject && (error_pos1 <=37) && decoder_en |-> double_ecc_error);
    ...
    ASSERT_NO_DOUBLE_ERROR_CORRECTION_IN_NO_SED_DED:   assert property (!sed_ded && single_error_inject && decoder_en |-> !double_ecc_error);    
    ...
    generate
      for (i = 0; i <= CHANNEL_WIDTH-1; i++) begin : loop_error_pos
        ASSERT_DATA_CORRECTION: assert property ((error_pos1 == i) && !sed_ded && decoder_en && single_error_inject |-> (encoded_data == decoded_data));
      end
    endgenerate  
